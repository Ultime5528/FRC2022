...
def iterate(value):
    ...
    preds = [to_pred_bbox(photo.name, color, box) for photo in dataset for color in Color for box in
             balldetection.algorithms.RANSAC(cached_images[photo.name], color, maxIterations=maxi, threshold_distance_percentage=d, threshold_inlier_count=inl)]
    ...
    metrics = {k: metrics[k] for k in ["maxi", "d", "inl", "Precision[blue]@.50", "Precision[red]@.50", "AveragePrecision@.50", "Recall[blue]@.50",
                               "Recall[red]@.50", "AverageRecall@.50", "F1[blue]@.50", "F1[red]@.50", "AverageF1@.50",
                               "Precision[blue]@.75",
                               "Precision[red]@.75", "AveragePrecision@.75", "Recall[blue]@.75", "Recall[red]@.75",
                               "AverageRecall@.75",
